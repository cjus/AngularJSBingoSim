...
angular.module('bingoftuiApp')
  .service('BingoUtilsService', function BingoUtilsService(ArrayUtilsService) {
    ...
    var columnOffsets = [1, 16, 31, 46, 61]; // column offset
    var columnLetters = ['B', 'I', 'N', 'G', 'O'];
    var TILE_PROP_SELECTED = 'selected';
    var TILE_PROP_BINGO = 'bingo';
    var MAX_BALLS = 75;
     ...
     *  ''=empty, 'selected'=number on this tile was called, 'bingo'=tile is part of bingo
     *  app data binds to a tile array in order to alter CSS properties for display.
     * @returns {Array}
     */
     ...
     * @name fillCard
     * @param card {Array}
     * @description populates the scope's bingo card with random values per bingo rules
     */
    ...
    this.checkPlay = function(ball, card, tiles) {
      var letterIndex = columnLetters.indexOf(ball[0]);
      var num = Number(ball.slice(1));
      for (var r = 0; r < 5; r++) {
